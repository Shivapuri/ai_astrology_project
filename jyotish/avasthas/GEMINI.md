...
1. **NO Friendship Calculation Here:** Avastha scripts (`deepti.py`, `lajjita.py`, etc.) MUST NOT calculate friendships themselves. They should either receive pre-calculated dignity/friendship strings from the master DAG (e.g., via `generate_jyotish.py`), or call `jyotish.relationships.relationships` directly.
