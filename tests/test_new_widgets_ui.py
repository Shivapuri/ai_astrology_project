...
def test_vimshottari_widget_renders(page: Page):
    ...
    assert page.locator("#cell2 .dasa-timeline-table tbody tr").count() >= 9
    assert page.locator("#cell2 .dasa-timeline-table tbody tr.md-row").count() == 9
...
def test_master_diagnostic_widget_renders(page: Page):
    init_page(page)
    ...
    assert v_select.is_visible()
    assert v_select.input_value() == "D1"

    # Switch to D9 Navamsha
    v_select.select_option("D9")
    page.wait_for_timeout(400)
    ...
    page.wait_for_timeout(400)
    subtitle_d10 = page.locator("#cell2 .varga-subtitle").inner_text()
    assert "D10" in subtitle_d10

def test_floating_master_diagnostic_modal(page: Page):
    init_page(page)
    page.evaluate("openFloatingMasterDiagnostic()")
    page.wait_for_timeout(500)
    modal = page.locator("#widgetMaximizeModal")
    assert modal.is_visible()
    title = page.locator("#widgetMaximizeModalTitle").inner_text()
